...
class Monitor(object):
    def __init__(self, client, verifier, hasher, db, cert_db, log_key,
        ...
        self.logging = LoggingWrapper(self.__log_server)
        ...
        try:
            ...
            self.logging.warning("Monitor state file not found, assuming first "
                            "run.")
        else:
            if not self.__state.HasField("verified_sth"):
                self.logging.warning("No verified monitor state, assuming first run.")
    ...
    def __update_state(self, new_state):
        ...
        self.logging.info("New state is %s" % new_state)
    ...
    def _set_pending_sth(self, new_sth):
        ...
        self.logging.info("STH verified, updating state.")
    ...
    def __verify_consistency_callback(self, proof, old_sth, new_sth):
        ...
        try:
            ...
            self.logging.error("Could not verify STH consistency: %s vs %s!!!\n%s" %
                          (old_sth, new_sth, e))
            raise
        else:
            self.__db.store_sth(self.servername,
    ...
    def __update_sth_errback(self, failure):
        ...
        response"""
        self.logging.error("get-sth from %s failed: %s" % (self.servername,
    ...
    def __handle_old_sth_errback(self, failure, sth_response):
        failure.trap(error.VerifyError)
        self.logging.error("Received older STH which is older and inconsistent "
    ...
    def __handle_old_sth_callback(self, result, sth_response):
        self.logging.warning("Rejecting received "
    ...
    def __update_sth_callback(self, sth_response):
        ...
        if self.__state.HasField("verified_sth"):
            if sth_response == self.__state.verified_sth:
                self.logging.info("Ignoring already-verified STH: %s" %
                             sth_response)
                return True
        ...
        try:
            ...
            self.logging.error("Invalid STH: %s" % sth_response)
            return False
    ...
    def _update_sth(self):
        ...
        self.logging.info("Fetching new STH")
    ...
    def _fetch_entries_errback(self, e, consumer):
        self.logging.error("get-entries from %s failed: %s" %
                      (self.servername, e))
        consumer.done(None)
        return True
    ...
    def _fetch_entries(self, start, end):
        ...
        self.logging.info("Fetching %d new entries: this will take %s..." %
                     (num_new_entries,
    ...
    def _update_result(self, updates_result):
        if not updates_result:
            self.logging.error("Update failed")
        return updates_result

    def update(self):
        """Update log view. Returns True if the update succeeded, False if any
        error occurred."""
        self.logging.info("Starting update for %s" % self.servername)
        d = self._update_sth()
